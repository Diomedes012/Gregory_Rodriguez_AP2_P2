...
import ucne.edu.gregory_rodriguez_ap2_p2.domain.usecase.GetGastosUseCase
...
class GastoListViewModel @Inject constructor(
    ...
    init {
        onEvent(GastoListEvent.LoadGastos)
    }

    fun onEvent(event: GastoListEvent) {
        when (event) {
            GastoListEvent.LoadGastos -> loadGastos()
        }
    }
    ...
    private fun loadGastos() {
        viewModelScope.launch {
            getGastosUseCase().collect { result ->
                when (result) {
                    is Resource.Success -> {
                        _state.update { it.copy(isLoading = false, gastos = result.data ?: emptyList()) }
                    }
                    is Resource.Error -> {
